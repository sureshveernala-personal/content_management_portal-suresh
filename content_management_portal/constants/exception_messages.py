INVALID_QUESTION_ID = (
    'Question id is not valid.', 'INVALID_QUESTION_ID'
)

INVALID_ROUGH_SOLUTION_ID = (
    'Rough Solution id is not valid.',
    'INVALID_ROUGH_SOLUTION_ID'
)

ROUGH_SOLUTION_NOT_BELONG_TO_QUESTION = (
    'Rough Solution id not belong to question.',
    'ROUGH_SOLUTION_NOT_BELONG_TO_QUESTION'
)


INVALID_PREFILLED_CODE_ID = (
    'Prefilled Code id is not valid.',
    'INVALID_PREFILLED_CODE_ID'
)

PREFILLED_CODE_NOT_BELONG_TO_QUESTION = (
    'Prefilled Code id not belong to question.',
    'PREFILLED_CODE_NOT_BELONG_TO_QUESTION'
)


INVALID_CLEAN_SOLUTION_ID = (
    'Clean Solution id is not valid.',
    'INVALID_CLEAN_SOLUTION_ID'
)

CLEAN_SOLUTION_NOT_BELONG_TO_QUESTION = (
    'Clean Solution id not belong to question.',
    'CLEAN_SOLUTION_NOT_BELONG_TO_QUESTION'
)

INVALID_USER_NAME = (
    'Invalid Credentials.', 'INVALID_USER_NAME'
)

INVALID_PASSWORD = (
    'Invalid Credentials.', 'INVALID_PASSWORD'
)

INVALID_OFFSET_VALUE = (
    'Invalid offset value.', 'INVALID_OFFSET_VALUE'
)

INVALID_LIMIT_VALUE = (
    'Invalid limit value.', 'INVALID_LIMIT_VALUE'
)

INVALID_TEST_CASE_ID = (
    'Test Case id is not valid.',
    'INVALID_TEST_CASE_ID'
)

TEST_CASE_NOT_BELONG_TO_QUESTION = (
    'Test case id not belong to question.',
    'TEST_CASE_NOT_BELONG_TO_QUESTION'
)

INVALID_HINT_ID = (
    'Hint id is not valid.',
    'INVALID_HINT_ID'
)

HINT_NOT_BELONG_TO_QUESTION = (
    'Hint id not belong to question.',
    'HINT_NOT_BELONG_TO_QUESTION'
)

INVALID_FIRST_TEST_CASE_ID = (
    'Test Case id is not valid.',
    'INVALID_FIRST_TEST_CASE_ID'
)

INVALID_SECOND_TEST_CASE_ID = (
    'Test Case id is not valid.',
    'INVALID_SECOND_TEST_CASE_ID'
)

INVALID_FIRST_HINT_ID = (
    'Hint id is not valid.',
    'INVALID_FIRST_HINT_ID'
)

INVALID_SECOND_HINT_ID = (
    'Hint id is not valid.',
    'INVALID_SECOND_HINT_ID'
)

INVALID_SOLUTION_APPROACH_ID = (
    'Solution Approach id is not valid.',
    'INVALID_SOLUTION_APPROACH_ID'
)

SOLUTION_APPROACH_NOT_BELONG_TO_QUESTION = (
    'Solution Approach id not belong to question.',
    'SOLUTION_APPROACH_NOT_BELONG_TO_QUESTION'
)

CAN_NOT_CREATE_MORE_THEN_ONE_SOLUTION_APPROACH = (
    'Can not create more then one solution approach to question.',
    'CAN_NOT_CREATE_MORE_THEN_ONE_SOLUTION_APPROACH'
)
